const executeOrder = () => {
    ...
    if(orderId.length >= 7 && !isNaN(orderId)){
        document.getElementById('order-valid').style.display = 'block'
        document.getElementById('order-invalid').style.display = 'none'
    document.getElementById('order-id-input-text').value = ''
    document.getElementById('order-id-display').innerHTML = `Your Order Status for ID: <b>${orderId}</b>`
   ...
   const chefReceivedOrder = () => new Promise((resolve, reject) => {
    ...
    document.getElementById('order-invalid').style.display = 'block'
    document.getElementById('order-valid').style.display = 'none'
}
}